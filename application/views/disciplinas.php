...
<form class="ui form column stackable grid" 
	action="<?=site_url()?>/disciplinas/salvar" method="post">
...
foreach($dados->ownDisciplinasalunosList as $da){
	...
	print "<td><a href='".site_url()."/alunos/index/{$aluno->id}'> {$aluno->nome} </a></td>";
...
foreach($listaPaginada["data"] as $ln){
	...
	print "<td><a href='".site_url()."/disciplinas/index/{$ln->id}'> Editar </a></td>";
	
	print "<td>{$ln->nome}</td>";

	print "<td><a onclick='confirmDelete(\"".site_url()."/disciplinas/deletar/{$ln->id}\")'> Deletar </a></td>";
